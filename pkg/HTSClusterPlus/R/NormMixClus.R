...
#' \item{nbClust.all }{Vector giving the number of clusters for each of the fitted models}
#' 
...
#  arg.user <- as.list(substitute(list(...)))[-1L]
  ...
  if (length(remainingK) > 0) {
    ...
    if(!parallel) {
      ...
      Kmods <- paste("K=", unlist(lapply(tmp, function(x) x$K)), sep="")
      # cat(Kmods, "\n")
      # cat(length(tmp), "\n")
  ...
  ICL.choose <- which(ICL.all == min(ICL.all, na.rm = TRUE))
  select.results <- all.results[[ICL.choose]]
  
  all.results <- lapply(all.results, function(xx) {
    if(is.null(xx) == TRUE) {
        xx <- list(probaPost = matrix(,nrow=0,ncol=0), 
  ...
  RES <- list(nbClust.all=nbClust.all,logLike.all = logLike.all, ICL.all = ICL.all, 
