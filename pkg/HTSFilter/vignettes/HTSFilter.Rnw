...
\date{Modified: November 7, 2012.   Compiled: \today}
\maketitle
...
To begin, we load the \Rpackage{HTSFilter} and \Rpackage{Biobase} packages, and attach the gene-level count data contained in \Robject{sultan}:
