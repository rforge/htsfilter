...
\date{Modified: August 8, 2013.   Compiled: \today}
\maketitle
...
The \Rpackage{HTSFilter} package allows for a straightforward integration within the \Rpackage{DESeq2} analysis pipeline, most notably allowing for $p$-values to be adjusted only for those genes passing the filter.
