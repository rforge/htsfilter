...
\date{Modified: July 24, 2014.   Compiled: \today}
\maketitle
...
%--------------------------------------------------
Normal combination and Fisher methods make differents assumptions on the model used. After using the two techniques, it can be seen which genes are identified by each single study or each combinaison method. The following script computes various indicators to assess the differences between each case.
<<IDDIRR>>=
IDDIRR<-function(finalde,deindst)
{
  DE=length(finalde)
...
<<calcul>>=
res=list()
res$study1=which(DEresults[["DE.study1"]]==1)
res$study2=which(DEresults[["DE.study2"]]==1)
res$fisher=which(DEresults[["DE.fishercomb"]]==1)
res$norm=which(DEresults[["DE.invnorm"]]==1)
res$AllStudies=which(DEresults[["DE.study1"]]==1 | DEresults[["DE.study2"]]==1)
IDDIRR(res$fisher,res$AllStudies)
IDDIRR(res$norm,res$AllStudies)
@
<<calcul2,eval=TRUE,echo=FALSE>>=
x=IDDIRR(res$fisher,res$AllStudies)
@
Thanks to Fisher method, meta-analysis gives $\Sexpr{x[["IDD"]]}$ ($\Sexpr{x[["IDR"]]}\%$) new genes and $\Sexpr{x[["Loss"]]}$ ($\Sexpr{x[["IRR"]]}\%$), are sidetracked.
\\
Venn diagram is also a more graphical to show this differences.
<<venndiagram,results=hide,echo=TRUE,eval=TRUE>>=
library(VennDiagram)
venn.plot<-venn.diagram(x = list(study1=res$study1,study2=res$study2,fisher=res$fisher,norm=res$norm),
...
\caption{Venn Diagram comparing the list of DE genes at a 5\% BH thereshold obtained by each individual study and the meta-analyse}
\end{center}
\end{figure}
Because of the use of simulated data, identified genes are spread over two studies.Moreover, it can be seen that there is similarities and exclusivities between the two methods.
