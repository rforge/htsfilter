...
\date{Modified: July 29, 2013.   Compiled: \today}
\maketitle
...
We recommand to store both p-value and Fold Change results in lists in order to perform meta-analysis and keep track of the potential conflicts (see section \ref{check})
<<storepvalandFC>>=
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width = .85\textwidth]{metaRNASeq_vignette-pvalDESeq2hist.png}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width = .85\textwidth]{metaRNASeq_vignette-pvalDEhist.png}
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width = .45\textwidth]{metaRNASeq_vignette-pvalfishcomb.png}
\includegraphics[width = .45\textwidth]{metaRNASeq_vignette-pvalinvnorm.png}
...
%When applying the procedure on the real data from two human melanoma cell lines \cite{Strub2011} as illustrated in \cite{Rau2013}, 6 contradictory genes were identified and removed.
